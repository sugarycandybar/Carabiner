...
class PlayitManager(EventEmitter):
    ...
    setup_url = "https://playit.gg/account/setup/wizard/new-account/third-party/third-party-code?partner=carabiner"
    ...
    def __init__(self):
        ...
        self.agent_name = f"carabiner ({platform.node()})"
        self.agent_web_url = ""
        self.max_tunnels = 4
    ...
    def install_latest_binary(self) -> tuple[bool, str]:
        ...
        try:
            ...
            req = urllib.request.Request(
                release_url,
                headers={"User-Agent": "Carabiner/1.0", "Accept": "application/vnd.github+json"},
            )
            ...
            req_bin = urllib.request.Request(download_url, headers={"User-Agent": "Carabiner/1.0"})
    ...
    def _create_tunnel(self, port: int = 25565, protocol: str = "tcp", label: str = "") -> Tunnel | None:
        ...
        tunnel_name = f"{safe_label}-{protocol}-{port}-{int(time.time()) % 100000}"
    ...
    def start(
        ...
        try:
            tunnel = self.get_tunnel(port, protocol=protocol, ensure=True, label="carabiner")
        except Exception as e:
            return False, str(e)
